class Solution:
    def medianNums(self, nums1, nums2):
        m = len(nums1)  # n: 2
        n = len(nums2)  # m：2
        # 让nums2成为更长的那一个数组
        if m > n:
        ...
        imin, imax = 0, m   # imin：0 imax：2
        while (imin <= imax):
            imid = imin + (imax - imin) // 2    # imid：1    nums的中位数index
            ...
            jmid = (m + n - 2 * imid) // 2  # jmid：1
            # 前面的判断条件只是为了保证不会index out of range
            if (imid > 0 and nums1[imid - 1] > nums2[jmid]): # 1 > 2
                # imid太大了，这是里精确查找，不是左闭右开，而是双闭区间，所以直接移动一位
                imax = imid - 1
            elif (imid < m and nums2[jmid - 1] > nums1[imid]):  #  -1 > 3
                imin = imid + 1
            # 满足条件
            else:
